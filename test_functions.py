...
def test_process_breport_files():
    ...
    assert df.loc["Ruminiclostridium cellulolyticum", df.columns[0]] == 107
...
def test_compute_relative_abundance():
    ...
    print(df.head()) 
    assert "Ruminiclostridium cellulolyticum" in df.index
    assert df.loc["Ruminiclostridium cellulolyticum", df.columns[0]] == 0.0020903475564227
...
def test_filter_low_abundance_species():
    ...
    assert df.loc["Bacteroides intestinalis", df.columns[0]] == 0.2203460755971263
